...
const Pomodoro: FC = () => {
  ...
  const [secund, setSecund] = useState<number>(60);
  const [minuteP, setMinuteP] = useState<number>(20);
  const [minuteR, setMinuteR] = useState<number>(20);
  const [minuteL, setMinuteL] = useState<number>(20);
  ...
  const handleStart = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const newIntervalId = setInterval(() => {
      setSecund((prevSecund) => prevSecund === 0 ? 60 : prevSecund - 1);
      setMinuteP((prevMinuteP) => secund === 1 ? prevMinuteP - 1 : prevMinuteP);
    }, 1000);
  };
          ...
          {active === 'pomodoro' && (
            <div className='P-3 bg-indigo-200 rounded-full w-[416px] h-[416px] pt-2 mt-10 m-auto'>
              <div className='w-[400px] rounded-full bg-indigo-50 h-[400px] mx-auto m-auto cursor-pointer hover:bg-indigo-100'>
                <h1 className='font-semibold text-[80px] text-indigo-600 pt-32'>
                  {minuteP}
                  :
                  {
                    secund === 60 ? '00' : secund
              ...
              <div>
                <button onClick={handleStart} className='w-2/3 bg-indigo-600 p-4 rounded-3xl mt-5 text-white text-2xl'>START</button>
              </div>
            </div>
          )}
